        ...
        <AppShell title="End of Day">
            ...
            <div className="eod-page-4a">
                <section className="eod-hero-4a">
                    <div>
                        <span className="module-kicker-4a">Daily records and handover</span>
                        <h1>End of Day</h1>
                        <p>{new Date(date).toLocaleDateString('en-GB', { weekday: 'long', day: 'numeric', month: 'long' })}</p>
                    </div>
                    <div className="eod-completion-4a">
                        <strong>{doneCount}</strong>
                        <span>of {rows.length} completed</span>
                ...
                {rows.length > 0 && (
                    <section className="eod-summary-4a">
                        <article><span>Records complete</span><strong>{doneCount}</strong></article>
                        <article><span>Still outstanding</span><strong>{rows.length - doneCount}</strong></article>
                        <article><span>Concerns flagged</span><strong>{rows.filter((row) => row.record?.concern).length}</strong></article>
                        <article><span>Incidents logged</span><strong>{rows.filter((row) => row.record?.incident).length}</strong></article>
                    </section>
                )}
                ...
                <section className="eod-grid-4a">
                    {rows.map((row) => (
                        <article key={row.id} className={`eod-member-card-4a ${row.record ? 'is-complete' : ''}`}>
                            <div className="eod-card-head-4a">
                                <div className="eod-avatar-4a">
                                    {row.name.split(' ').map((part) => part.charAt(0)).join('').slice(0, 2)}
                                </div>
                                <div className="eod-flags-4a">
                                    {row.record?.incident && <span title="Incident logged">🩹 Incident</span>}
                                    {row.record?.concern && <span className="is-concern" title="Concern flagged">⚠ Concern</span>}
                                </div>
                            </div>
                            <h2>{row.name}</h2>
                            <div className="eod-mood-journey-4a">
                                <div><span>Arrival</span><strong>{row.arrival_mood ? MOOD_EMOJI[row.arrival_mood] : '—'}</strong></div>
                                <i>→</i>
                                <div><span>Departure</span><strong>{row.record?.end_mood ? MOOD_EMOJI[row.record.end_mood] : '—'}</strong></div>
                            </div>
                            <div className="eod-record-state-4a">
                                {row.record ? (
                                    <><strong>Record complete</strong><span>{row.record.session_type || 'Daily record saved'}</span></>
                                ) : (
                                    <><strong>Record required</strong><span>Add activities, outcomes and departure details.</span></>
                                )}
                            </div>
                            <button onClick={() => open(row)} className={row.record ? 'is-edit' : ''}>
                                {row.record ? '✓ Review and edit' : 'Complete daily record →'}
                            </button>
                        </article>
                    ))}
                </section>
